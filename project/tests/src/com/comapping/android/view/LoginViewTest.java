...
public class LoginViewTest extends AndroidTestCase {
	...
	public void testGoingDownFromUp() {
		assertEquals("down should be next focus from up", password,
				mFocusFinder.findNextFocus(mRoot, eMail, View.FOCUS_DOWN));
		assertEquals("down should be next focus from up", check, mFocusFinder
				.findNextFocus(mRoot, password, View.FOCUS_DOWN));
	}
